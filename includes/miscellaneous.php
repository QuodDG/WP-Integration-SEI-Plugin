...
function qdo_isei_save_option($option_name, $value) {
    ...
    return is_null(get_option($option_name, null)) ? add_option($option_name, $striped_value) :
            ( get_option($option_name) !== $striped_value ? update_option($option_name, $striped_value) : true );
}
...
function qdo_isei_verify_recaptcha_v3(string $response) {
    //return anr_verify_captcha($response);
    $anr_skey = anr_get_option('secret_key');
    $r = QdoJsonRequest::post((object) [
                        'url' => 'https://www.google.com/recaptcha/api/siteverify',
                        'body' => [
                            'secret' => $anr_skey,
                            'response' => $response,
                            'remoteip' => qdo_isei_get_user_ip()
                        ]
    ]);
    if (!isset($r->success)) {
        return false;
